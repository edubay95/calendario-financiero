...
Versión mejorada:
 ...
 - SOLUCIONADO (Futuros): Usa yfinance para Ex-Div/Earnings y 
   FinancialModelingPrep (FMP) para Pagos Futuros FIABLES.
...
# --- ¡NUEVO! Carga la API Key desde los Secretos de GitHub ---
...
# --- ¡NUEVA FUNCIÓN! ---
def fetch_future_payments_fmp(ticker, api_key):
    """
    Obtiene PAGOS de dividendos futuros y confirmados desde FinancialModelingPrep
    """
    if not api_key:
        print(f"[AVISO] {ticker}: No se encontró FMP_API_KEY. Omitiendo búsqueda de pagos futuros.")
        return []
    ...
    try:
        ...
        # FMP puede devolver un diccionario vacío si no hay datos
        ...
        for div_info in data['historical']:
            pay_date = safe_parse_date(div_info.get('paymentDate'))
            amount = div_info.get('dividend')
            
            # Buscamos pagos futuros que tengan fecha e importe
            if pay_date and amount and pay_date > today:
                future_payments.append( (pay_date, float(amount)) )
        
        return future_payments
        
    except Exception as e:
        print(f"[AVISO] {ticker}: Error al contactar API de FMP (Pagos Futuros): {e}")
        return []

# --- FUNCIÓN PRINCIPAL (MODIFICADA) ---
def fetch_info_for_ticker(ticker, fmp_api_key):
    """
    Obtiene info del ticker:
    - yfinance: Nombre, Resultados (pasado/futuro), Ex-Div (pasado/futuro)
    - yfinance: Historial de Pagos (PASADO)
    - FMP: Historial de Pagos (FUTURO)
    """
    tk = yf.Ticker(ticker)
    out = {
        'shortName': None,
    ...
    # 1. Info general (de yfinance)
    try:
        info = tk.info
        out['shortName'] = info.get('shortName') or info.get('longName')
    except Exception as e:
        print(f"[AVISO] {ticker}: No se pudo cargar 'info' (yfinance): {e}")

    # 2. DATOS FUTUROS (de yfinance, excepto pagos)
    try:
        cal = tk.calendar
        ...
        if not cal.empty:
            ...
            # Futura Fecha Ex-Dividendo (Fiable)
            ...
            # Ya no leemos 'Dividend Date' de aquí, era el dato erróneo.

    except Exception as e:
        print(f"[AVISO] {ticker}: No se pudo cargar 'calendar' (yfinance): {e}")

    # 3. DATOS HISTÓRICOS (de yfinance)
    ...
    try:
        ...
        print(f"[AVISO] {ticker}: No se pudo cargar 'earnings_dates' (yfinance): {e}")

    # Historial de Pagos de Dividendos (Fecha de PAGO - PASADO)
    try:
        divs = tk.dividends # Fuente histórica fiable
        if not divs.empty:
            for d, amount in divs.items():
                pay_date = safe_parse_date(d)
                if pay_date:
                    out['dividends_history'].append( (pay_date, float(amount)) )
    except Exception as e:
        print(f"[AVISO] {ticker}: No se pudo cargar 'dividends' (yfinance): {e}")
    ...
    try:
        ...
        print(f"[AVISO] {ticker}: No se pudo cargar 'actions' (yfinance): {e}")
        
    # 4. --- ¡NUEVO! DATOS FUTUROS (de FMP) ---
    # Obtenemos los pagos futuros de FMP y los añadimos a la lista
    future_divs_fmp = fetch_future_payments_fmp(ticker, fmp_api_key)
    if future_divs_fmp:
        print(f"[INFO] {ticker}: Encontrados {len(future_divs_fmp)} pagos futuros en FMP.")
        out['dividends_history'].extend(future_divs_fmp)

    # 5. Limpiar Nones y duplicados
...
def build_events_from_holdings(csv_path=HOLDINGS_CSV, api_key=FMP_API_KEY):
    df = pd.read_csv(csv_path, dtype=str)
    df['cantidad'] = pd.to_numeric(df['cantidad'], errors='coerce').fillna(0).astype(int)
    events = [] 

    # Verificamos la API Key una vez
    if not api_key:
        print("="*50)
        print("[AVISO GLOBAL] No se ha proporcionado una API Key de FinancialModelingPrep (FMP_API_KEY).")
        print("El script solo mostrará dividendos pasados.")
        print("="*50)
    ...
    for _, row in df.iterrows():
        try:
            ...
            # Pasamos la API key a la función
            info = fetch_info_for_ticker(ticker, api_key)
            ...
            # 1️⃣ Ex-Dividendo (Historial + Futuro de yfinance)
            ...
            # 2️⃣ Dividendo (Historial de yfinance + Futuro de FMP)
            ...
            # 3️⃣ Resultados (earnings) (Historial + Futuro de yfinance)
...
if __name__ == "__main__":
    ...
    # El script ahora busca la API key que el robot le pasa
    all_events = build_events_from_holdings(HOLDINGS_CSV, FMP_API_KEY)
